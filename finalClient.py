	...
	try:
		...
		for alarms in listofalarms:
			...
			if "Alarm" in alarm_string:
				alarmstoprint =  alarmstoprint + alarm_string[15:].replace("'}","") + "\n"
				print alarmstoprint
			else:
				for days, dayskey in alarms.iteritems():
					...
					for time, sleepquality in dayskey.iteritems():
						...
						sleepqualitytoprint = sleepqualitytoprint + days + " " + time + " " + sleepquality + "\n" 

		with open("led_alarms.txt", 'w') as f:
			f.write(alarmstoprint)

		with open("led_sleepquality.txt", 'w') as f:
			f.write(sleepqualitytoprint)
